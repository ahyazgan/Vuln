...
async def main() -> None:
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://live") as c:
        ...
        comp = (await c.post(f"{P}/auth/register", json={
            "email": "sec@company.com", "password": "password123",
            "role": "company", "tenant_name": "AcmeCorp"})).json()
        ch = {"Authorization": f"Bearer {comp['access_token']}"}
        prog = await c.post(f"{P}/programs", headers=ch, json={
            "name": "Acme Web", "scope_domains": ["example.com", "*.example.com"],
        ...
        print("\n== hacker registers ==")
        hack = (await c.post(f"{P}/auth/register", json={
            "email": "h4x@research.com", "password": "password123",
            "role": "hacker", "tenant_name": "RedTeam"})).json()
        ...
        s1 = await c.post(f"{P}/scans", headers=hh, json={
            "target_url": "https://api.example.com/login", "program_id": program_id,
            "scan_level": 6})
        ...
        check(f"queued scope = {enqueued[0]['scope_domains']}",
              enqueued[0]["scope_domains"] == ["example.com", "*.example.com"])

        print("\n== out-of-scope scan is refused before queueing (§7.2) ==")
        s2 = await c.post(f"{P}/scans", headers=hh, json={
            "target_url": "https://evil.com/", "program_id": program_id})
        ...
        check(f"company GET /scans -> {forbidden.status_code} (role forbidden)",
