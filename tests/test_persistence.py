...
def _result(scan_id: str) -> PipelineResult:
    findings = [
        FindingBase(
            title="Verbose server header", severity=Severity.LOW, cvss_score=2.0, description="d"
        ),
        FindingBase(
            title="Reflected parameter", severity=Severity.MEDIUM, cvss_score=5.0, description="d"
        ),
    ]
    chained = [
        ChainedFinding(
            title="Reflected XSS via leaked context",
...
async def test_persist_writes_tenant_scoped_findings(session):
    ...
    rows = (
        (await session.execute(select(ScanFinding).where(ScanFinding.tenant_id == job.tenant_id)))
        .scalars()
        .all()
    )
    assert len(rows) == 3
